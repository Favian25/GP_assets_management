...
const emptyForm = {
  kodeAksesoris: "", namaAksesoris: "", kategori: "", merek: "", model: "",
  lokasi: "", kondisi: "", keterangan: "",
  jumlahUnit: "", hargaAset: "", tanggalPembelian: "", jenisAset: "",
};
...
export default function AksesorisPage() {
  ...
  const fetchDependencies = async () => {
    try {
      const cats = await getAllCategories("aksesoris");
      const brds = await getAllBrands("aksesoris");
  ...
  const fetchData = useCallback(async () => {
    try { 
      ...
      setError(null); 
      const data = await getAllAksesoris();
      setAssets(data || []); 
    }
    catch { setError("Gagal memuat data aksesoris."); }
    finally { 
      setLoading(false); 
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      if (search.trim()) { try { setAssets(await searchAksesoris(search.trim())); setCurrentPage(1); } catch { } }
      else { fetchData(); }
    }, 400);
  ...
  const handleCreateKat = async (e) => {
    ...
    try { setSubmitting(true); await createCategory(newKatData.nama, newKatData.kode_singkat, "aksesoris"); showToast("Kategori ditambahkan!"); setShowKatModal(false); setNewKatData({nama:"",kode_singkat:""}); await fetchDependencies(); }
  ...
  const handleCreateMerek = async (e) => {
    ...
    try { setSubmitting(true); await createBrand(newMerekData.nama, "aksesoris"); showToast("Merek ditambahkan!"); setShowMerekModal(false); setNewMerekData({nama:""}); await fetchDependencies(); }
  ...
  const handleCreate = async (e) => {
    e.preventDefault();
    if (!formData.namaAksesoris) { showToast("Nama Aksesoris wajib diisi", "error"); return; }
    try { setSubmitting(true); await createAksesoris({ ...formData, kodeAksesoris: undefined }, imageFile); showToast("Aksesoris berhasil ditambahkan!"); setShowModal(false); setFormData(emptyForm); setImageFile(null); setImagePreview(null); await fetchData(); }
    catch (err) { showToast(err.response?.data?.message || "Gagal menambahkan aksesoris", "error"); }
    finally { setSubmitting(false); }
  };
  const handleUpdate = async (e) => {
    e.preventDefault();
    if (!editFormData.namaAksesoris) { showToast("Nama Aksesoris wajib diisi", "error"); return; }
    try { setSubmitting(true); await updateAksesoris(showEdit.id, editFormData, editImageFile); showToast("Aksesoris berhasil diperbarui!"); setShowEdit(null); setEditFormData(emptyForm); setEditImageFile(null); setEditImagePreview(null); await fetchData(); }
    catch (err) { showToast(err.response?.data?.message || "Gagal memperbarui aksesoris", "error"); }
    finally { setSubmitting(false); }
  };
  const handleUpdateKondisi = async () => {
    if (!showKondisiModal || !newKondisi) return;
    try { setSubmitting(true); await updateAksesorisKondisi(showKondisiModal.id, newKondisi); showToast("Kondisi berhasil diperbarui!"); setShowKondisiModal(null); await fetchData(); }
    catch (err) { showToast(err.response?.data?.message || "Gagal memperbarui kondisi", "error"); }
    finally { setSubmitting(false); }
  };
  const handleDelete = async () => {
    if (!showDeleteConfirm) return;
    try { setSubmitting(true); await deleteAksesoris(showDeleteConfirm.id); showToast("Aksesoris berhasil dihapus!"); setShowDeleteConfirm(null); await fetchData(); }
    catch (err) { showToast(err.response?.data?.message || "Gagal mengHapus Aksesoris", "error"); }
    finally { setSubmitting(false); }
  };
  const openEdit = (item) => {
    setEditFormData({
      kodeAksesoris: item.kodeAksesoris || "", namaAksesoris: item.namaAksesoris || "",
      kategori: item.kategori || "", merek: item.merek || "", model: item.model || "",
      lokasi: item.lokasi || "", kondisi: item.kondisi || "", keterangan: item.keterangan || "",
      jumlahUnit: item.jumlahUnit || "", hargaAset: item.hargaAset || "",
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {isEdit && (
            <div>
              <label className="mb-1.5 block text-sm font-medium text-slate-700">Kode Aksesoris</label>
  ...
  if (loading) {
    return (
      <div>
        <div className="mb-6"><h1 className="text-2xl font-bold text-slate-800">Daftar Aksesoris</h1><p className="text-sm text-slate-500">Kelola data aksesoris Galeria Karya Media</p></div>
  ...
  return (
    <div>
      ...
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-800">Daftar Aksesoris</h1>
        <p className="text-sm text-slate-500">Kelola data aksesoris Galeria Karya Media</p>
      </div>
      ...
      <div className="relative overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm border-t-4 border-t-primary">
        ...
        <div className="px-5 py-4 border-b border-slate-200 bg-gradient-to-r from-primary/5 to-primary/10">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <p className="text-xs text-slate-600 font-medium">Total Harga Aksesoris</p>
            ...
            <div className="text-xs text-slate-500">
              {brandFilter ? `Filter: ${brandFilter === "Galeria studio" ? "Galeria Studio" : "Galeria Production"}` : "Semua Brand"} • {filteredData.length} aksesoris
            </div>
          </div>
        </div>
        ...
        <div className="overflow-x-auto border-t border-slate-100">          <table className="w-full text-left text-sm table-fixed">
            <thead>
              <tr className="border-t border-t-slate-300 border-b border-b-slate-300">
                <th className="w-[140px] px-4 py-3 font-bold text-slate-700 text-center text-xs border-r border-slate-200"><button onClick={() => handleSort("kodeAksesoris")} className="cursor-pointer flex items-center justify-center uppercase tracking-wider w-full">Kode <SortIcon columnKey="kodeAksesoris" sortConfig={sortConfig} /></button></th>
                ...
                <th className="w-[150px] px-4 py-3 font-bold text-slate-700 text-center text-xs border-r border-slate-200"><button onClick={() => handleSort("namaAksesoris")} className="cursor-pointer flex items-center justify-center uppercase tracking-wider w-full">Nama <SortIcon columnKey="namaAksesoris" sortConfig={sortConfig} /></button></th>
                <th className="w-[110px] px-3 py-3 font-bold text-slate-700 text-center text-xs border-r border-slate-200"><button onClick={() => handleSort("jumlahUnit")} className="cursor-pointer flex items-center justify-center uppercase tracking-wider w-full">Jumlah <SortIcon columnKey="jumlahUnit" sortConfig={sortConfig} /></button></th>
                ...
                <th className="w-[110px] px-3 py-3 font-bold text-slate-700 text-center text-xs border-r border-slate-200"><button onClick={() => handleSort("merek")} className="cursor-pointer flex items-center justify-center uppercase tracking-wider w-full">Merek <SortIcon columnKey="merek" sortConfig={sortConfig} /></button></th>
            ...
            <tbody>
              {paginatedData.map((item, index) => (
                <tr key={item.id} className={`border-b border-slate-100 transition-colors ${index % 2 === 0 ? "bg-slate-100" : "bg-white"}`}>
                  <td className="w-[140px] px-4 py-3 font-mono text-xs font-semibold text-slate-700 hover:text-primary cursor-pointer truncate border-r border-slate-200 align-middle text-center" onClick={() => setShowDetail(item)}>{item.kodeAksesoris}</td>
                  ...
                  <td className="w-[150px] px-4 py-3 text-xs text-slate-600 hover:text-primary cursor-pointer truncate border-r border-slate-200 align-middle" onClick={() => setShowDetail(item)}>{item.namaAksesoris}</td>
                  <td className="w-[110px] px-3 py-3 text-xs border-r border-slate-200 align-middle">
                    <div className="flex flex-col gap-0.5">
                      <span className="font-semibold text-emerald-600">{item.jumlahUnit ?? "-"} Tersisa</span>
                      <span className="text-slate-500 text-[11px]">dari {item.jumlahTotal ?? item.jumlahUnit ?? "-"}</span>
                    </div>
                  </td>
              ...
              {assets.length === 0 && (<tr><td colSpan={9} className="px-5 py-10 text-center text-slate-400">Tidak ada data aksesoris ditemukan.</td></tr>)}
            </tbody>
          </table>
        </div>
            ...
            <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/40 p-4 transition-opacity animate-in fade-in duration-300" onClick={() => setShowModal(false)}>
              <form onSubmit={handleCreate} className="max-h-[90vh] w-full max-w-2xl flex flex-col rounded-2xl bg-white shadow-xl border-t-4 border-t-emerald-500 animate-modal-in" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between border-b border-slate-100 bg-emerald-50 px-6 py-4 rounded-t-2xl shrink-0">
                  <h2 className="text-lg font-bold text-emerald-800">Tambah Aksesoris Baru</h2>
                ...
                <div className="flex items-center justify-end gap-3 border-t border-slate-100 px-6 py-4 bg-white rounded-b-2xl shrink-0">
                  ...
                  <button type="submit" disabled={submitting} className="cursor-pointer rounded-lg bg-emerald-500 px-4 py-2 text-sm font-medium text-white shadow-sm transition-colors hover:bg-emerald-600 disabled:opacity-60">{submitting ? "Menyimpan..." : "Simpan Aksesoris"}</button>
                </div>
              </form>
            </div>
          )}
            ...
            <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/40 p-4 transition-opacity animate-in fade-in duration-300" onClick={() => setShowDetail(null)}>
              <div className="max-h-[90vh] w-full max-w-lg flex flex-col rounded-2xl bg-white shadow-xl border-t-4 border-t-blue-500 animate-modal-in" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between border-b border-slate-100 bg-blue-50 px-6 py-4 rounded-t-2xl shrink-0">
                  <h2 className="text-lg font-bold text-blue-800">Detail Aksesoris</h2>
                ...
                <div className="p-6 space-y-5 overflow-y-auto custom-scrollbar flex-1">
                  {getImageUrl(showDetail.gambar) && (
                    <div className="flex justify-center">
                      <div className="relative h-48 w-full max-w-[300px] cursor-pointer hover:ring-2 hover:ring-blue-400 transition-all rounded-xl overflow-hidden border border-slate-200" onClick={() => setLightboxImg(getImageUrl(showDetail.gambar))}>
                        <Image src={getImageUrl(showDetail.gambar)} alt={showDetail.namaAksesoris} fill unoptimized className="object-contain" />
                      </div>
                    </div>
                  )}
                  <div>
                    <h4 className="mb-4 text-sm font-bold text-slate-800">Informasi Aksesoris</h4>
                    <div className="grid grid-cols-2 gap-4 md:gap-6">
                      {[
                        ["Kode Aksesoris", showDetail.kodeAksesoris],
                        ["Nama Aksesoris", showDetail.namaAksesoris],
                        ["Model", showDetail.model],
                        ["Kategori", showDetail.kategori],
                        ["Merek", showDetail.merek],
                        ["Jenis Brand", showDetail.jenisAset],
                        ["Jumlah", `${showDetail.jumlahUnit ?? "-"} Tersisa dari Total ${showDetail.jumlahTotal ?? showDetail.jumlahUnit ?? "-"}`],
                        ["Harga Aksesoris", formatRupiah(showDetail.hargaAset)],
                  ...
                  <div>
                    <h4 className="mb-4 text-sm font-bold text-slate-800">Lokasi & Kondisi</h4>
                    <div className="grid grid-cols-1 gap-4 md:gap-6">
                      {[["Lokasi", showDetail.lokasi], ["Kondisi", showDetail.kondisi], ["Keterangan", showDetail.keterangan]].map(([l, v]) => (
                        <div key={l} className="flex flex-col gap-1.5">
                          <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">{l}</span>
                ...
                <div className="flex items-center justify-end border-t border-slate-100 px-6 py-4 gap-3 bg-white rounded-b-2xl shrink-0">
                  <button onClick={() => {openEdit(showDetail); setShowDetail(null);}} className="cursor-pointer rounded-lg bg-amber-500 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-amber-600 shadow-sm">Edit Aksesoris</button>
            ...
            <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/40 p-4 transition-opacity animate-in fade-in duration-300" onClick={() => setShowEdit(null)}>
              <form onSubmit={handleUpdate} className="max-h-[90vh] w-full max-w-2xl flex flex-col rounded-2xl bg-white shadow-xl border-t-4 border-t-amber-500 animate-modal-in" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between border-b border-slate-100 bg-amber-50 px-6 py-4 rounded-t-2xl shrink-0">
                  <h2 className="text-lg font-bold text-amber-800">Edit Aksesoris</h2>
          ...
          {showDeleteConfirm && (
            <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/40 p-4 transition-opacity animate-in fade-in duration-300" onClick={() => setShowDeleteConfirm(null)}>
              <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl animate-modal-in" onClick={(e) => e.stopPropagation()}>
                ...
                <h3 className="mb-2 text-center text-lg font-bold text-slate-800">Hapus Aksesoris?</h3>
                <p className="mb-6 text-center text-sm text-slate-500">
                  Apakah Anda yakin ingin menghapus Aksesoris <strong>{showDeleteConfirm.namaAksesoris}</strong>? Seluruh data riwayat terkait aset ini akan ikut terhapus.
                </p>
                <div className="flex gap-3">
                  ...
                  <button onClick={handleDelete} disabled={submitting} className="cursor-pointer flex-1 rounded-lg bg-rose-600 px-4 py-2 text-sm font-medium text-white shadow-sm transition-colors hover:bg-rose-700 disabled:opacity-60">{submitting ? "Menghapus..." : "Hapus Aksesoris"}</button>
                </div>
              </div>
            </div>
          )}
          ...
          {showKondisiModal && (
            <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/40 p-4 transition-opacity animate-in fade-in duration-300" onClick={() => setShowKondisiModal(null)}>
              <div className="w-full max-w-sm rounded-2xl bg-white shadow-xl border-t-4 border-t-blue-500 animate-modal-in" onClick={(e) => e.stopPropagation()}>
                <div className="p-6">
                  <h3 className="text-lg font-bold text-slate-800 mb-1">Ubah Kondisi Aksesoris</h3>
                  <p className="text-sm text-slate-500 mb-4 truncate">{showKondisiModal.namaAksesoris}</p>
