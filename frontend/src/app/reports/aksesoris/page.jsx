...
export default function AksesorisReportPage() {
  ...
  const [sortConfig, setSortConfig] = useState({ key: "namaAksesoris", direction: "asc" });
  ...
  const fetchData = async () => {
    try {
      ...
      setError(err.message || "Gagal memuat data aksesoris");
    } finally {
      setLoading(false);
    }
  };
  ...
  const getUniqueValues = (key) => {
    return [...new Set(aksesoris.map(a => a[key]).filter(Boolean))].sort();
  };
  ...
  const getColumns = () => [
    ...
    { header: "Unit", dataKey: "jumlahUnit" },
